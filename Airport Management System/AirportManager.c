...
int		saveManagerToFile(const AirportManager* pManager, const char* fileName)
{
	FILE* fp;
	fp = fopen(fileName, "w");
	CHECK_NULL__MSG_CLOSE_FILE(fp, fp, "Error open airport manager file to write");
	fprintf(fp, "%d\n", pManager->count);
	NODE* pNode = pManager->airportList.head.next;
	while (pNode)
	{
		int ans = saveAirportToFile(pNode->key, fp);
		CHECK_0_MSG_CLOSE_FILE(ans, fp, "Error write airport");
...
int		loadManagerFromFile(AirportManager* pManager, const char* fileName)
{
	...
	fp = fopen(fileName, "r");
	CHECK_NULL__MSG_CLOSE_FILE(fp, fp, "Error open airport manager file");
	...
	for (int i = 0; i < pManager->count; i++)
	{
		...
		CHECK_MSG_RETURN_0(pPort, "Allocation error");
